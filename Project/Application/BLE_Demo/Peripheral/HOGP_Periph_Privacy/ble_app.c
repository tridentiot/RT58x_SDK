...
#endif
static uint8_t app_privacy_en = 0;
...
static void app_peripheral_handler(app_req_param_t *p_param)
{
    ...
    case APP_REQUEST_PRIVACY_ONOFF:
    {
        ...
        if (app_privacy_en == 0)
        {
            p_privacy_cfg.host_id = host_id;
            p_privacy_cfg.privacy_mode = NETWORK_PRIVACY_MODE;
            //p_privacy_cfg.privacy_mode = DEVICE_PRIVACY_MODE;

            status = ble_cmd_privacy_enable(&p_privacy_cfg);

            if (status == BLE_ERR_OK)
            {
                info_color(LOG_CYAN, "privacy enable success\n");
                app_privacy_en = 1;
            }
            else
            {
                info_color(LOG_RED, "privacy enable fail, %d\n", status);
            }
        }
        else
        {
            status = ble_cmd_privacy_disable();
            if (status == BLE_ERR_OK)
            {
                info_color(LOG_CYAN, "privacy disable success\n");
                app_privacy_en = 0;
            }
            else
            {
                info_color(LOG_RED, "privacy disable fail, %d\n", status);
            }
        }
...
static ble_err_t adv_init(void)
{
    ...
    do
    {
        ...
        if (app_privacy_en != 0)
        {
            adv_param.own_addr_type = RANDOM_RESOLVABLE_ADDR;
        }
        else
        {
...
void privacy_status_change(void)
{
    app_request_set(APP_HID_P_HOST_ID, APP_REQUEST_PRIVACY_ONOFF, true);
}

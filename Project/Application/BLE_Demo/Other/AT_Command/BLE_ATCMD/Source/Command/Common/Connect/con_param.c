...
static ble_err_t read_cmd(atcmd_item_t *item)
{
    ...
    {
        printf("id=%d min=%u max=%u latency=%u timeout=%u\n",
               i,
               con_param[i].min_conn_interval,
               con_param[i].max_conn_interval,
               con_param[i].periph_latency,
               con_param[i].supv_timeout);
    }
    item->status = AT_CMD_STATUS_OK;
    return BLE_ERR_OK;
}

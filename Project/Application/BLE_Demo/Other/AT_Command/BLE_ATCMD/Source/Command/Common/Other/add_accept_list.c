...
static void add_accept_list_init(cmd_info_t *this);
static ble_err_t set_cmd(atcmd_item_t *item);
static void test_cmd(atcmd_item_t *item);

// PUBLIC VARIABLE DECLARE
cmd_info_t add_accept_list =
{
    .cmd_name = "+ADDFAL",
    .description = "add filter accept list",
    .init = add_accept_list_init
};
...
static ble_err_t set_cmd(atcmd_item_t *item)
{
    ...
    if (item->param_length == 2)
    {
        ...
        ble_err_t status = ble_cmd_add_device_to_accept_list((ble_filter_accept_list_t *)&list_addr);

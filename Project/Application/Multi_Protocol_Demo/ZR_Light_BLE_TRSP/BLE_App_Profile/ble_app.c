...
#define DEVICE_NAME                     '0', 'A', '0', 'B', '0', 'C', '0', 'D', '0', 'E', '0', 'F', '1', '0', '1', '1' /* default ZB mac address*/
...
static const ble_gap_addr_t  DEVICE_ADDR = {.addr_type = RANDOM_STATIC_ADDR,
                                            .addr = {0xC1, 0xB4, 0xA2, 0xA2, 0xB6, 0xC1 }
                                           };
...
static ble_err_t adv_disable(void);
...
static void ble_svcs_trsps_evt_handler(ble_evt_att_param_t *p_param)
{
    if (p_param->gatt_role == BLE_GATT_ROLE_SERVER)
    {
        ...
        case BLESERVICE_TRSPS_UDATRW01_WRITE_WITHOUT_RSP_EVENT:
            if (strncmp((char *)p_param->data, LEAVE_STR, sizeof(LEAVE_STR) - 1) == 0)
            {
                zigbee_app_evt_change(APP_ZB_LEAVE_EVT, false);
            }
...
static void app_peripheral_handler(app_req_param_t *p_param)
{
    ...
    case APP_REQUEST_ADV_STOP:
        if (ble_app_link_info[host_id].state == STATE_ADVERTISING)
        {
            status = adv_disable();
            if (status != BLE_ERR_OK)
            {
                info_color(LOG_RED, "adv_disable() status = %d\n", status);
                break;
            }
        }
        else
        {
            info_color(LOG_YELLOW, "adv already stop\n");
        }
        break;
...
static ble_err_t adv_init(void)
{
    ...
    memcpy(&adv_scan_rsp_data[2], DEVICE_NAME_STR, sizeof(DEVICE_NAME_STR));
...
{
    memcpy(DEVICE_NAME_STR, p_new_device_name, sizeof(DEVICE_NAME_STR));
}

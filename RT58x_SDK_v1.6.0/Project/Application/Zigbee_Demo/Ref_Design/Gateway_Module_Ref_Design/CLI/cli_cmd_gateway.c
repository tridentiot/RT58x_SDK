...
static int
_cli_cmd_zcl_color_temp(int argc, char **argv, cb_shell_out_t log_out, void *pExtra)
{
    zigbee_zcl_data_req_t *pt_data_req;
    uint16_t addr, color_temp;
    uint8_t ep;
    ...
    do
    {
        ...
        if (pt_data_req)
        {
            pt_data_req->cmdFormat[0] = color_temp & 0xFF;
            pt_data_req->cmdFormat[1] = (color_temp >> 8) & 0xFF;
            pt_data_req->cmdFormat[2] = 0x00;
            pt_data_req->cmdFormat[3] = 0x00;
            zigbee_zcl_request(pt_data_req, pt_data_req->cmdFormatLen);
            sys_free(pt_data_req);
        }
...
const sh_cmd_t  g_cli_cmd_color_temp __cli_cmd_pool =
{
    .pCmd_name      = "ctemp",
    .cmd_exec       = _cli_cmd_zcl_color_temp,
    .pDescription   = "ctemp\n"
    "  usage: ctemp [addr] [ep] [color temp]\n"
    "    e.g. ctemp 0xabcd 2 454\n",
};

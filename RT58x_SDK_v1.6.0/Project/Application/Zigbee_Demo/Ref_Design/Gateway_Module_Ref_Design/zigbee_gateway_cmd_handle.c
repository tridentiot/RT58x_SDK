...
{
    uint32_t cmd_index;
    uint32_t cmdID = ((gateway_cmd_pd *)(&pBuf[5]))->command_id;
    ...
    if (cmdID == RAF_CMD_PERMIT_JOIN_REQUEST)
    {
        timeoutNum = ((gateway_cmd_pd *)(&pBuf[5]))->parameter[0];
        if (timeoutNum != 0)
        {
            info("permit join start! Duration: %d s!\n", timeoutNum);
            permit_join_flag = true;
        }
        else
        {
            info("permit join stop! Duration: %d s!\n", timeoutNum);
...
zigbee_gateway_cmd_parser(uint8_t *pBuf,
                          uint16_t plen,
                          uint16_t *datalen,
                          uint16_t *offset)
{
    ...
    do
    {
        ...
        if (1)
        {
            totalLen = (sizeof(gateway_cmd_hdr) + hdr->len + sizeof(gateway_cmd_end));
            ...
            info("| ");
            /* show Length(1) */
            info("%02X ", pBuf[idx + (sizeof(gateway_cmd_hdr) - 1)]);
            info("| ");

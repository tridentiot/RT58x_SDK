...
static uint8_t identify_onoff;
//=============================================================================
//                Private Function Declaration
//=============================================================================

void plug_Led1_on(void)
{
    gpio_pin_write(5, 1);
    Delay_ms(5);
    gpio_pin_write(5, 0);
...
void plug_Led2_off(void)
{
    gpio_pin_write(29, 1);
    Delay_ms(5);
    gpio_pin_write(29, 0);
}
void plug_Led3_on(void)
{
    gpio_pin_write(30, 1);
    Delay_ms(5);
    gpio_pin_write(30, 0);
...
{
    info_color(LOG_GREEN, "EP:0x%02x , onoff:%d\n", ep, on_off);
    set_on_off_status(ep, on_off);
    switch (ep)
    {
    case BUTTON_1_EP:
    {
    ...
    do
    {
        ...
        else if (cmd == ZB_ZCL_CMD_SCENES_STORE_SCENE)
        {
            ...
            else
            {
                ...
                scene_table_db.scene_table[valid_scene_idx].onoff_stat = get_on_off_status(BUTTON_1_EP);
        ...
        else if (cmd == ZB_ZCL_CMD_SCENES_RECALL_SCENE)
        {
            ...
            else
            {
                ...
                set_on_off_status(BUTTON_1_EP, onoff);
...
static void _zcl_onoff_process(uint16_t cmd,
                               uint16_t datalen,
                               uint8_t *pdata,
                               uint8_t ep)
{
    info_color(LOG_GREEN, "get ep status 0x%02x  ep2 :0x%02x ep3:0x%02x\n", get_on_off_status(BUTTON_1_EP), get_on_off_status(BUTTON_2_EP), get_on_off_status(BUTTON_3_EP));

    //if(cmd == 2){  //just for debug
    //  Led_status = 1 - Led_status;
    //}
    Light_key_onoff_process(ep, get_on_off_status(ep));
}

void zigbee_zcl_msg_handler(sys_tlv_t *pt_tlv)
{
    zigbee_zcl_data_idc_t *pt_zcl_msg = (zigbee_zcl_data_idc_t *)pt_tlv->value;
    info("[Multi_enpoint] Recv ZCL message 0x%04X\n", pt_zcl_msg->dstAddr);
    info("[Multi_enpoint] Cluster %04x, cmd %02x, endpoint :%02x, is_common_command %02x\n", pt_zcl_msg->clusterID, pt_zcl_msg->cmd, pt_zcl_msg->dstEndpint, pt_zcl_msg->is_common_command );
    ...
    do
    {
        ...
        //if (pt_zcl_msg->dstEndpint != BUTTON_2_EP)
        // {
        //     break;
        //}
        ...
        else if (pt_zcl_msg->is_common_command == 0)
        {
            ...
            case ZB_ZCL_CLUSTER_ID_ON_OFF:
                _zcl_onoff_process(pt_zcl_msg->cmd, pt_zcl_msg->cmdFormatLen, pt_zcl_msg->cmdFormat, pt_zcl_msg->dstEndpint);
                break;
            default:
                break;
            }

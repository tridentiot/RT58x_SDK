...
static ble_err_t set_cmd(atcmd_item_t *item)
{
    ...
    if (item->param_length == 4)
    {
        ...
        ble_err_t status = ble_cmd_gatt_read_rsp(&data_param);
...
{
    printf(
        "+READRSP = <num1>, <num2>, <num3>, <raw>\n"
        "  send read response for specific host ID\n"
        "    <num1> : host ID\n"
        "      range : 0-1\n"
        "    <num2> : service handle value\n"
        "      range : 0-65535\n"
        "    <num3> : the num of read response length\n"
        "      range : 0-244\n"
        "    <raw> : raw data\n"
        "    notice: the num of read response value length should lower than (mtu size - 3)\n"
    );
}

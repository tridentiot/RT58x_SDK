...
static void _zdo_evt_rejoin(sys_tlv_t *pt_tlv)
{
    if ((zigbee_app_join_continue(APP_ZB_REJOIN) == TRUE) && (ble_fota_state_get() == OTA_STATE_IDLE))
    {
...
static void _zdo_evt_start(sys_tlv_t *pt_tlv)
{
    ...
    if (pt_start_idc->status != 0)
    {
        ...
        if ((zigbee_app_join_continue(APP_ZB_JOIN) == TRUE) && (ble_fota_state_get() == OTA_STATE_IDLE))
        {
